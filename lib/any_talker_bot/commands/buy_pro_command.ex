defmodule AnyTalkerBot.BuyProCommand do
  ...
  """
  def call(%Reply2{context: %{update: %{message: msg}}} = reply) do
    user = reply.context.extra.user

    case Accounts.get_current_subscription(user) do
      %Subscription{} = sub ->
        Reply2.send_message(reply, already_subscribed_message(sub), for_dm: true)
  ...
  def handle_successful_payment(
    ...
    case Accounts.activate_pro_subscription(user) do
      {:ok, sub} ->
        Reply2.send_message(reply, activation_success_message(sub), for_dm: true)

      {:error, error} ->
        log_activation_error(user.id, error)
        Reply2.send_message(reply, activation_error_message(), for_dm: true)
    end
  end
  ...
  defp send_invoice_or_error(reply, chat_id) do
    case send_invoice(chat_id) do
      ...
      {:error, error} ->
        ...
        Reply2.send_message(reply, "Что-то не работает, попробуй позже", for_dm: true)
    end
  end
